...
module.exports = settings => {
  ...
  const resolvers = mapValues(Resolvers, resolver => resolver({ models, jwt, emailer, keycloak }));

  const changelog = Changelog(models.Changelog);

  const eachHour = Scheduler('hour');

  eachHour(expire(models));
  ...
  return (message, done) => {
    ...
    Promise.resolve()
      ...
      .then(body => {
        ...
        console.log(`Resolving: model ${body.model}, action: ${body.action}, id: ${body.id}`);
