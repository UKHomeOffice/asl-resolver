      ...
      .then(profile => {
        ...
        return Promise.resolve()
          ...
          .then(token => {
            return Promise.resolve()
              .then(() => {
                return Invitation.queryWithDeleted().upsert({
                    ...pick(args, ['establishmentId', 'profileId', 'role']),
                    deleted: null,
                    token
                  }, {
    ...
    return Promise.resolve()
      ...
      .then(model => {
        return Promise.resolve()
          ...
          .then(() => {
            return Permission.queryWithDeleted()
              .where({
          ...
          .then(count => {
            if (!count) {
              ...
              return Permission.query()
                .insert({
                  ...pick(model, ['establishmentId', 'profileId', 'role'])
                })
                .returning('*');
            }
          })
          .then(() => Invitation.query().where({ token: data.token }).delete());
      });
  }
