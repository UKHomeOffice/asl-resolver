...
module.exports = ({ models }) => async ({ action, data = {}, id }, transaction) => {
  ...
  if (action === 'update') {
    ...
    const removedRoles = place.roles.filter(r => !requestedRoles.includes(r.id)).map(r => r.id);

    await PlaceRole.query(transaction).where({ placeId: id }).whereIn('roleId', removedRoles).delete();
