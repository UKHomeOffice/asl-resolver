...
async function renameProtocolLocation(ProjectVersion, transaction, establishmentId, renameFrom, renameTo) {
  ...
  await Promise.all(versionData.rows.map(async ({id: versionId, data}) => {
    data.protocols.forEach(protocol => {
      if (protocol.locations?.includes(renameFrom)) {
        protocol.locations = protocol.locations.filter(location => location !== renameFrom);
        if (!protocol.locations.includes(renameTo)) {
          protocol.locations.push(renameTo);
        }
      }
    });

    await ProjectVersion.query(transaction).where({ id: versionId }).update({ data });
  }));
}
...
module.exports = ({ models }) => async ({ action, data, id }, transaction) => {
  ...
  if (action === 'rename-protocol-location') {
    const {renameFrom, renameTo} = data;

    const establishment = Establishment.query(transaction).findById(id);
    await renameProtocolLocation(ProjectVersion, transaction, id, renameFrom, renameTo);
