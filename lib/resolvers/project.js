...
module.exports = ({ models }) => async ({ action, data, id, meta = {}, changedBy }, transaction) => {
  ...
  const deleteOrphanedReminders = async (project, version) => {
    ...
    const grantedConditionKeys = [...grantedProjectConditions, ...grantedProtocolConditions].map(c => c.key);
  ...
  if (action === 'suspend') {
    return Project.query(transaction).patchAndFetchById(id, { suspendedDate: new Date().toISOString() });
  }

  if (action === 'reinstate') {
    return Project.query(transaction).patchAndFetchById(id, { suspendedDate: null });
  }
