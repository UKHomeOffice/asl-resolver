...
module.exports = ({ models }) => async ({ action, data, id, meta = {}, changedBy }, transaction) => {
  ...
  const deleteOrphanedReminders = async (project, grantedVersion) => {
    const existingReminders = await Reminder.query(transaction).where({ modelId: project.id });
    const grantedConditions = (grantedVersion.data.conditions || []).map(c => c.key);
    const grantedProtocolConditions = flattenDeep(
      (grantedVersion.data.protocols || []).map(p => (p.conditions || []).map(c => c.key))
    );
  ...
  const transferReminders = async (oldProject, newProject) => {
    return Reminder.query(transaction).patch({ modelId: newProject.id }).where({ modelId: oldProject.id });
  };
  ...
  if (action === 'grant') {
    ...
    await deleteOrphanedReminders(project, grantedVersion);
