const { pick, get, omit, isEqual, isInteger, flatten } = require('lodash');
...
const calculateExpiryDate = require('../helpers/calculate-expiry-date.js');
  ...
  async ({ action, data, id, meta = {}, changedBy }, transaction) => {
    ...
    const fork = (preserveStatus) => {
      ...
      return Promise.resolve()
        ...
        .then((version) =>
          ProjectVersion.query(transaction).insertAndFetch(
            pick(version, fields)
          )
        );
    };
    ...
    const deleteProject = () => {
      return Promise.resolve()
        ...
        .then(() =>
          ProjectVersion.query(transaction).delete().where({ projectId: id })
        )
    ...
    function cleanVersionData(version) {
      ...
      data.establishments = (data.establishments || []).filter(
        (e) => !e.deleted
      );

      // remove additional establishments data if granting without any AA selected
      const additionalEstablishmentsSelected = get(
        version,
        'data.other-establishments',
        false
      );
    ...
    function setAdditionalAvailability(version) {
      ...
      return Promise.resolve()
        ...
        .then(() =>
          ProjectEstablishment.query(transaction)
            .where({ projectId: id })
    ...
    function transferAdditionalAvailability(version, projectId) {
      ...
      return ProjectEstablishment.query(transaction)
        .where({ projectId })
        .insert(
          establishments.map((establishmentId) => {
            return {
              projectId,
              establishmentId,
              status: 'active',
    ...
    const deleteOrphanedReminders = async (project, version) => {
      const existingReminders = await Reminder.query(transaction).where({
        modelId: project.id
      });
      const grantedProjectConditions = version.data.conditions || [];
      const grantedProtocolConditions = flatten(
        (version.data.protocols || []).map((p) => p.conditions || [])
      );

      const grantedConditionKeys = [
        ...grantedProjectConditions,
        ...grantedProtocolConditions
      ].map((c) => c.key);

      return Promise.all(
        existingReminders.map((reminder) => {
          return grantedConditionKeys.includes(reminder.conditionKey)
            ? Promise.resolve()
    ...
    if (action === 'convert') {
      ...
      versionToGrant = await versionToGrant
        .$query(transaction)
        .patchAndFetch({ status: 'granted', data });
    ...
    if (action === 'fork-ra') {
      ...
      ra = ra || { projectId: id };
      return RetrospectiveAssessment.query(transaction)
        .insertAndFetch(pick(ra, fields))
        .then((version) => {
          return { id: version.id };
        });
    }
    ...
    if (action === 'submit-draft') {
      ...
      if (project.status === 'inactive') {
        ...
        await project
          .$query(transaction)
          .patchAndFetch({ species: species.length ? species : null });
      }
    ...
    if (action === 'sync-training') {
      ...
      const training = await Certificate.query(transaction).where({
        profileId: project.licenceHolderId
      });
      await version
        .$query(transaction)
        .patchAndFetch({ data: { ...version.data, training } });
    ...
    if (action === 'grant-ra') {
      const raVersionToGrant = await getMostRecentVersion(
        null,
        RetrospectiveAssessment
      );
      const project = await Project.query(transaction).findById(id);
    ...
    if (action === 'grant') {
      ...
      const versionData = cleanVersionData(versionToGrant);
      ...
      if (!isEqual(currentDuration, newDuration) || newDuration === undefined) {
        patch.expiryDate = calculateExpiryDate(patch.expiryDate, newDuration);
      }

      patch.raDate = getRaDate(
        versionToGrant,
        patch.expiryDate || project.expiryDate
      );
    ...
    if (action === 'update') {
      ...
      const patchVersionData = (version) => {
        ...
        const experienceFields = {
          ...pick(meta, EXPERIENCE_FIELDS),
          ...pick(data, EXPERIENCE_FIELDS)
        };
        const patch = {
          data: {
            ...version.data,
      ...
      return Project.query(transaction)
        ...
        .then((project) => {
          if (project.status === 'active') {
            return fork(true).then(patchVersionData);
          }
          if (project.status === 'inactive') {
            return getMostRecentVersion('draft').then(patchVersionData);
          }
        })
        .then(() =>
          Project.query(transaction).patchAndFetchById(
            id,
            pick(data, ['licenceHolderId', 'amendedDate'])
          )
        );
    }
    ...
    if (action === 'delete-amendments') {
      return Promise.resolve()
        ...
        .then((project) => {
          ...
          return Promise.resolve()
            .then(() =>
              ProjectVersion.query(transaction)
                .where({ projectId: id })
                .orderBy('createdAt', 'desc')
            )
            .then((versions) => {
            ...
            .then((recentDraftVersions) => {
              return ProjectVersion.query(transaction)
                ...
                .whereIn(
                  'id',
                  recentDraftVersions.map((v) => v.id)
                );
            })
    ...
    if (action === 'delete-ra' && raVersionId) {
      return Promise.resolve()
        .then(() =>
          RetrospectiveAssessment.query(transaction)
            .findById(raVersionId)
            .delete()
        )
        .then(() => Project.query(transaction).findById(id));
    }
    ...
    if (action === 'transfer') {
      ...
      await ProjectEstablishment.query(transaction)
    ...
    if (action === 'transfer-draft') {
      ...
      const profile = await Profile.query(transaction)
        .findById(project.licenceHolderId)
    ...
    if (action === 'withdraw') {
      return ProjectVersion.query(transaction)
        ...
        .then((version) => {
          return version
            .$query(transaction)
            .patchAndFetch({ status: 'withdrawn' });
        })
        .then(() => Project.query(transaction).findById(id));
    }
    ...
    if (action === 'refuse') {
      ...
      if (project.status !== 'inactive') {
        throw new Error('Can only refuse inactive projects');
      }

      const refusedDate = new Date().toISOString();

      return Project.query(transaction).patchAndFetchById(id, { refusedDate });
    }
