...
module.exports = ({ models }) => async ({ action, data, id, meta = {} }, transaction) => {
  ...
  function getRaDate(data, startDate) {
    if (data.retrospectiveAssessment || data.retrospectiveAssessmentRequired) {
  ...
  if (action === 'grant') {
    ...
    patch.raDate = getRaDate(versionToGrant.data, patch.expiryDate || versionToGrant.expiryDate);
  ...
  if (action === 'revoke') {
    ...
    const revocationDate = newDate().toISOString();

    const patch = {
      status: 'revoked',
      revocationDate,
      raDate: getRaDate(latestVersion.data, revocationDate)
    };
