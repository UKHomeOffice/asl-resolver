...
module.exports = ({ models }) => async ({ action, data, id, meta = {} }, transaction) => {
  ...
  const calculateExpiryDate = (issueDate, duration) => {
    ...
    duration.years = parseInt(get(duration, 'years'), 0);
    duration.months = parseInt(get(duration, 'months'), 0);
  ...
  if (action === 'create') {
    ...
    if (isLegacyStub) {
      const projectStub = {
        ...
        status: 'active',
        expiryDate: calculateExpiryDate(data.issueDate, version.data.duration),
        schemaVersion: 0,
        isLegacyStub: true,
        version: {
          ...version,
