...
module.exports = ({ models }) => async ({ action, data, id, meta = {}, changedBy }, transaction) => {
  const { Project, ProjectVersion, ProjectEstablishment, Profile, Certificate, RetrospectiveAssessment, Establishment } = models;
  ...
  function cleanVersionData(version, primaryEstablishment) {
    ...
    const projectLocations = [
      ...data.establishments.map(establishment => establishment.name),
      ...(data.polesList || []).map(pole => pole.title),
      data.transferToEstablishmentName || primaryEstablishment.name
    ];

    // remove any protocol locations which are no-longer valid
    data.protocols.forEach(protocol => {
      if (protocol.locations) {
        protocol.locations = protocol.locations.filter(location => projectLocations.includes(location));
      }
    });
  ...
  if (action === 'grant') {
    ...
    const primaryEstablishment = await Establishment.query(transaction).findById(project.establishmentId);
    const data = cleanVersionData(versionToGrant, primaryEstablishment);
  ...
  if (action === 'transfer') {
    ...
    const newVersion = await ProjectVersion.query(transaction).insert({
      ...
      status: 'granted',
      data: cleanVersionData(versionToTransfer, primaryEstablishment)
    });
