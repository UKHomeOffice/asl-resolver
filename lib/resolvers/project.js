...
const { generateLicenceNumber, normaliseProjectVersion, getRetrospectiveAssessment } = require('../utils');
...
module.exports = ({ models }) => async ({ action, data, id, meta = {} }, transaction) => {
  ...
  if (action === 'grant') {
    ...
    const ra = getRetrospectiveAssessment(versionToGrant.data);
    versionToGrant.data.retrospectiveAssessment = ra.required || ra.condition;
    versionToGrant.data.retrospectiveAssessmentRequired = ra.required;
    ...
    // add date for retrospective assessment
    if (ra.required || ra.condition) {
      patch.raDate = moment(patch.expiryDate).add(6, 'months').toISOString();
    }
  ...
  if (action === 'revoke') {
    ...
    const ra = getRetrospectiveAssessment(latestVersion);
    ...
    if (ra.required || ra.condition) {
      patch.raDate = moment(patch.revocationDate).add(6, 'months').toISOString();
    } else {
      patch.raDate = null;
    }
