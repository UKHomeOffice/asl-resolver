...
const { applicantTrainingUseAtWork } = require('../../../asl-pages/pages/pil/unscoped/courses/participants/add/schema');
...
module.exports = ({ models }) => async ({ action, data, id }, transaction) => {
  ...
  if (action === 'create') {
    ...
    return TrainingPil.query(transaction).insert(
      {
        profileId: profile.id,
        trainingCourseId: data.trainingCourseId,
        organisation: data.organisation,
        qualificationLevelAndSubject: data.qualificationLevelAndSubject,
        applicantLearningUse: data.applicantLearningUse,
        jobTitleOrQualification: data.jobTitleOrQualification,
        fieldOfExpertise: data.fieldOfExpertise,
        applicantTrainingUseAtWork: data.applicantTrainingUseAtWork,
