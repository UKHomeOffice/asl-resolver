  ...
  function updateAdditionalAvailability(version) {
    ...
    function removeRelations(projectEstablishments) {
      ...
      return Promise.all(projectEstablishments.map(pe => {
        if (pe.status === 'draft') {
          return pe.$query(transaction).hardDelete()
        }
        return Promise.resolve();
      }));
    }

    function reactivateRemovedRelations(projectEstablishments) {
      if (!projectEstablishments.length) {
        return Promise.resolve();
      }
      return Promise.all(projectEstablishments.map(pe => {
        return pe.$query(transaction).patch({ versionId: null, status: 'draft' });
      }));
    }
    ...
    return ProjectEstablishment.query(transaction).where({ projectId: version.projectId })
      .then(projectEstablishments => {
        ...
        const relationsToReactivate = projectEstablishments.filter(pe => {
          return pe.status === 'removed' && establishments.includes(pe.establishmentId);
        });

        return Promise.all([
          addRelations(establishmentIdsToAdd),
          removeRelations(relationsToRemove),
          reactivateRemovedRelations(relationsToReactivate)
        ])
          .then(() => version);
      });
  }
